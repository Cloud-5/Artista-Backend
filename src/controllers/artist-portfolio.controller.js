...
class ArtistPortfolio {
  static fetchArtistDetails(artistId, customerId) {
    ...
    COALESCE(COUNT(DISTINCT a.artwork_id), 0) AS total_creations,  
    u.description,
    u.profile_photo_url,
    u.banner_img_url,
    CASE 
        ...
        ELSE 0 
    END AS is_following,  
    COALESCE(AVG(ar.rating_value), 0) AS average_rating 
FROM
    user u
LEFT JOIN
...
LEFT JOIN
    (
        ...
        GROUP BY 
            a.artist_id
    ) likes_count ON u.user_id = likes_count.artist_id 
WHERE
    u.user_id = ?
GROUP BY
    u.user_id;
`,
  ...
  static getSocial(artistId) {
    return db.execute("call GetSocialAccounts(?)", [artistId]);
  }
}
...
exports.getArtistDetails = async (req, res, next) => {
  ...
  console.log("artistId:", artistId, "customerId:", customerId);
  try {
    const artistDetails = await ArtistPortfolio.fetchArtistDetails(
      artistId,
      customerId
    );
